...
stopWords =[]
sw = open('stopwords_portugues.txt', 'r')
for line in sw:
    line2 = line.strip()
...
def limpeza (twt):
    #for tweet in Tweets:
        ...
        # Jeferson como aqui será em portugês e os contraction são em inglês então retirei esta linha
...
try:
    ...
    sql_teste_tweets = 'SELECT * FROM TweetsOfLabeledNews where tweet_limpo is null'
      
    #Construindo função para receber variáveis e concatená-las com SQL
    def buildQuery (twl, i):   
        return ("UPDATE TweetsOfLabeledNews SET Tweet_limpo = '"+ str(twl)+"' WHERE TweetID = "+ str(i))
    ...
    for row in tqdm(result_tweets):
        ...
        #arousal = row[5]
        #dominance = row[6]       
        tweetl = row[9] # coluna criada/usada para persistir        
        #ID = row[5]
        ...
        cursor_twtlimpo_column.execute(buildQuery(tweet_limpo, row[3])) 
