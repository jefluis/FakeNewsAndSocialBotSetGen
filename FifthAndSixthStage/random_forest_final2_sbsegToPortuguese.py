...
joblib.dump(clf_reduced, 'modelo_randomforest_reduced_features.pkl')
 
'''
Finding Important Features in Scikit-learn
'''
feature_imp_basic = pd.Series(clf_reduced.feature_importances_,index=basic_features_twitter).sort_values(ascending=False).head(10)
...
joblib.dump(clf, 'modelo_randomforest_original_features.pkl')
...
feature_imp = pd.Series(clf.feature_importances_,index=features).sort_values(ascending=False).head(10)
...
clf_expanded = RandomForestClassifier(n_estimators=100, random_state=42, class_weight='balanced')
...
joblib.dump(clf_expanded, 'modelo_randomforest_expanded_features.pkl')
 
'''
Finding Important Features in Scikit-learn
'''
 
feature_imp_tang = pd.Series(clf_expanded.feature_importances_,index=features).sort_values(ascending=False).head(10)
...
clf_VAD_Tang = RandomForestClassifier(n_estimators=100, random_state=42, class_weight='balanced')
...
# Salvar o modelo treinado com features expandidas
joblib.dump(clf_VAD_Tang, 'modelo_randomforest_VAD_Tang_features.pkl')
 
'''
Finding Important Features in Scikit-learn
'''
 
feature_imp_VAD_Tang = pd.Series(clf_VAD_Tang.feature_importances_,index=features).sort_values(ascending=False).head(10)
