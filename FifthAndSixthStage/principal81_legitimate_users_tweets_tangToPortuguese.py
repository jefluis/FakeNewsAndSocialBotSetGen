...
def buildQuery(i, d_values):
    update_query = "UPDATE legitimate_users_tweets_original_pt SET "
...
try:
    ...
    cursor_legitimate_users_tweets = db.cursor(pymysql.cursors.DictCursor)
    cursor_tang_columns = db.cursor()

    # Montando SQL statements    
    sql_espaco_emocional_tang = 'SELECT * FROM espaco_emocional_tang'
    sql_teste_tweets = 'SELECT * FROM legitimate_users_tweets_original_PT WHERE Control2=0'
