...
#positive_class_weight = sum(y_train_reduced == 0) / max(sum(y_train_reduced == 1), 1)

num_positive = y_train_reduced.value_counts().get(1, 0)
num_humano = y_train_reduced.value_counts().get(0, 0)
positive_class_weight = num_humano / max(num_positive, 1)
...
joblib.dump(clf_reduced, 'modelo_xgboost_reduced_features.pkl')
 
'''
Finding Important Features in XGBoost
'''
feature_imp_basic = pd.Series(clf_reduced.feature_importances_,index=basic_features_twitter).sort_values(ascending=False)
...
# Treinar o modelo XGBoost com as features originais
clf = XGBClassifier(objective='binary:logistic', random_state=42, scale_pos_weight=sum(y_train_reduced == 0) / sum(y_train_reduced == 1))
...
joblib.dump(clf, 'modelo_XGBoost_original_features.pkl')
...
First, you need to create a XGBoost model.
...
# Treinar o modelo XGBoost com um conjunto expandido de features
...
clf_expanded = XGBClassifier(objective='binary:logistic', random_state=42, scale_pos_weight=sum(y_train_reduced == 0) / sum(y_train_reduced == 1))
clf_expanded.fit(X_train_expanded, y_train_expanded)
 
# Avaliar a precisão do modelo no conjunto de teste com features expandidas
y_pred_expanded = clf_expanded.predict(X_test_expanded)
...
joblib.dump(clf_expanded, 'modelo_XGBoost_expanded_features.pkl')
 
'''
Finding Important Features in Scikit-learn
'''
 
feature_imp_tang = pd.Series(clf_expanded.feature_importances_,index=features).sort_values(ascending=False)
...
clf_VAD_Tang = XGBClassifier(objective='binary:logistic', random_state=42, scale_pos_weight=sum(y_train_reduced == 0) / sum(y_train_reduced == 1))
clf_VAD_Tang.fit(X_train_VAD_Tang, y_train_VAD_Tang)
 
# Avaliar a precisão do modelo no conjunto de teste com features expandidas
y_pred_VAD_Tang = clf_VAD_Tang.predict(X_test_VAD_Tang)
...
# Salvar o modelo treinado com features expandidas
joblib.dump(clf_VAD_Tang, 'modelo_XGBoost_VAD_Tang_features.pkl')
 
'''
Finding Important Features in Scikit-learn
'''
 
feature_imp_VAD_Tang = pd.Series(clf_VAD_Tang.feature_importances_,index=features).sort_values(ascending=False)
