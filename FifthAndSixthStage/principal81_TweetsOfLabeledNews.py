...
def build_query(valence, arousal, dominance, tweet_id):
    return f"UPDATE TweetsOfLabeledNews SET Val = {valence}, Aro = {arousal}, Dom = {dominance}, Control1 = 1 WHERE TweetID = {tweet_id}"
...
def process_tweets(cursor_vad_columns, vads, tweets):
    ...
    for tweet in tqdm(tweets):
        ...
        palavras = tweet['Tweet_limpo'].split()
...
try:        
    ...
    cursor_TweetsOfLabeledNews = db.cursor(pymysql.cursors.DictCursor)
    ...
    #Esta query abaixo se fosse usar o VA feito no Brasil
    #sql_espaco_emocional = 'SELECT Id_Word, Word, V_Mean_Sum, A_Mean_Sum, D_Mean_Sum FROM espaco_emocional_vad_anew_br'
    sql_teste_tweets = 'SELECT * FROM TweetsOfLabeledNews WHERE Control1=0' 
    ...
    cursor_TweetsOfLabeledNews.execute(sql_teste_tweets)
    tweets = cursor_TweetsOfLabeledNews.fetchall()
