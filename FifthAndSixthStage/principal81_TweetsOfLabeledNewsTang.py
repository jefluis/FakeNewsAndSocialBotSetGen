...
a) Para cada Tweet da tabela LEGITIMATE_POLLUTERS_TWEETS, verificar se as palavras contidas nele estão na tabela ESPAÇO EMCOCIONAL 
b) Calcular os valores médios de V-A-D de cada Tweet (usando os valores V-A_D de cada palavra encontrada)   
c) Persistir Valores Médios de V-A-D de cada Tweet na tabela LEGITIMATE_POLLUTERS_TWEETS do banco de dados IME
...
def buildQuery(i, d_values):
    update_query = "UPDATE TweetsOfLabeledNews SET "
...
def process_tweet(row, tang):
    twid = row['TweetID']        
    palavras = row['Tweet_limpo'].split()
...
#===============================================================================
# def CarregarDicionarioTang ():
#     # Crie um cursor
#     cursor = db.cursor(pymysql.cursors.DictCursor)
#     
#     # Defina o tamanho do lote para paginação
#     tamanho_lote = 10000
#     
#     # Inicialize uma variável para armazenar os resultados
#     resultados_totais = []
#     
#     # Execute a consulta SQL em lotes usando paginação
#     offset = 0
#     while True:
#         sql_query = f"SELECT * FROM sua_tabela LIMIT {tamanho_lote} OFFSET {offset}"
#         cursor.execute(sql_query)
#         resultados_lote = cursor.fetchall()
#     
#         # Se não houver mais resultados, pare o loop
#         if not resultados_lote:
#             break
#     
#         # Adicione os resultados do lote ao dicionário
#         for linha in resultados_lote:
#             resultados_totais.append(linha)
#     
#         # Aumente o valor do offset para a próxima página
#         offset += tamanho_lote
#     
...
#     db.close()
#     
#     # Agora, resultados_totais contém todos os registros da sua consulta SQL
#     print(f"Total de registros encontrados: {len(resultados_totais)}")
#===============================================================================
...
try:
    ...
    cursor_users_tweets = db.cursor(pymysql.cursors.DictCursor)
    cursor_tang_columns = db.cursor()

    # Montando SQL statements    
    sql_espaco_emocional_tang = 'SELECT * FROM espaco_emocional_tang'
    sql_teste_tweets = 'SELECT * FROM TweetsOfLabeledNews WHERE Control2=0'
...
'D41': row['D41'], 'D42': row['D42'], 'D43': row['D43'], 'D44': row['D44'], 'D45': row['D45'], 'D46': row['D46'], 'D47': row['D47'], 'D48': row['D48'], 'D49': row['D49'], 'D50': row['D50']
    ...
    cursor_users_tweets.execute(sql_teste_tweets)
    result_tweets = cursor_users_tweets.fetchall()    
